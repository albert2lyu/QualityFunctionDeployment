    ...
    ui(new Ui::Step6_1)
{
    ...
    QStringList m_Header,c_Header;
    m_Header<<QString("质量参数1")<<QString("质量参数2")<<QString("质量参数3")<<QString("质量参数4");
    c_Header<<QString("质量参数1")<<QString("质量参数2")<<QString("质量参数3");
    ui->qTableWidget->setHorizontalHeaderLabels(m_Header);//添加横向表头
    ui->qTableWidget->setVerticalHeaderLabels(c_Header);//添加横向表头
    ...
    HStrList.push_back(QString("质量参数3"));
    HStrList.push_back(QString("质量参数4"));
    ...
    for(int i=0;i<ColumnNum*RowNum*2;i++)
    {
        ...
        { QComboBox *tmp= new QComboBox();
            ...
            tmp->addItem("类型六");
            Combox.append(tmp);}
        else if(i%2==1){QComboBox *tmp= new QComboBox();
            tmp->addItem("1");
            tmp->addItem("2");
            tmp->addItem("3");
            tmp->addItem("4");
...
void Step6_1::on_pushButton_5_clicked()
{
    ...
    QString filename =  QDir::tempPath() + QDir::separator() +QCoreApplication::applicationName() + "Step6_1_temp" + "xls";
    if(filename.isEmpty())
        return;
    QFile file(filename);
